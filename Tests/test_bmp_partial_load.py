    ...
    def test_correctness_random_crops(self, temp_bmp_large):
        ...
        for _ in range(20):
            ...
            np.testing.assert_array_equal(
                result1, expected, err_msg=f"load_region failed for box {crop_box}"
            )
            np.testing.assert_array_equal(
                result2,
                expected,
...
class TestBmpPartialLoadDifferentBitDepths:
    ...
    def _create_bmp_and_test(self, mode, size=(100, 100)):
        ...
        if mode in ("L", "P"):
            ...
            data = np.arange(size[0] * size[1] * 3, dtype=np.uint8).reshape(
                size[0], size[1], 3
            )
            img = Image.fromarray(data, mode="RGB")
        elif mode == "RGBA":
            data = np.arange(size[0] * size[1] * 4, dtype=np.uint8).reshape(
                size[0], size[1], 4
            )
            img = Image.fromarray(data, mode="RGBA")
        else:
